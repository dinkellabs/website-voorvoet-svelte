...
function makeLoadArgs(lang: string) {
  const fullUrl = `https://voorvoet.nl/${lang}/order-insoles`;
  ...
  return {
    ...
    setHeaders: vi.fn(),
    platform: undefined,
    route: { id: '/[lang=lang]/order-insoles' },
    depends: vi.fn(),
...
describe('[lang=lang]/order-insoles/+page.server.ts', () => {
  it('returns form, meta, and alternates for en/order-insoles', async () => {
  ...
  it('throws 404 when nl tries to use order-insoles slug', async () => {
